...
def forward_noflashattn_window(
    ...
    window_size = int(q_len * group_size_ratio)
    ...
    query_states = query_states.reshape(bsz, self.num_heads, q_len // window_size, window_size, self.head_dim)
    key_states = key_states.reshape(bsz, self.num_heads, q_len // window_size, window_size, self.head_dim)
    value_states = value_states.reshape(bsz, self.num_heads, q_len // window_size, window_size, self.head_dim)
...
def replace_llama_attn(use_flash_attn=True, use_full=False, inference=False):
    if use_flash_attn:
        ...
        transformers.models.llama.modeling_llama.LlamaAttention.forward = forward_noflashattn_window
